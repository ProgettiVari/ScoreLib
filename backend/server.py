...
app.add_middleware(
    ...
    # Only vercel.live itself (Vercel's own toolbar/comments widget) is
    # allowed by pattern. The previous regex also matched *any* *.vercel.app
    # or *.preview.emergentagent.com subdomain -- both are public multi-tenant
    # platforms where anyone can host a page, so that effectively trusted
    # every other project on those platforms too. Add real extra origins via
    # the BACKEND_CORS_ORIGINS env var instead of widening this pattern.
    allow_origin_regex=r"^https://vercel\.live$",
...
async def issue_login_otp(email: str) -> bool:
    ...
    Deliberately does NOT fall back to FormSubmit: that service can only
    forward to the site's own fixed inbox, never to the actual user, so
    falling back to it here would either leak the code to the wrong inbox
    or silently fail to reach the person logging in.
    """
